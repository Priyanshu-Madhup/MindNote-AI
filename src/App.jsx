      ...
      {isOpen && (
        ...
        >
          <div className="flex flex-col w-full">
            {/* Brand Logo */}
            <div className="px-6 mb-6 flex items-center gap-2 text-xl font-bold tracking-tighter text-neutral-100 pb-5 border-b border-[#2A2A2A]">
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.aside
          ...
          transition={{ type: 'spring', stiffness: 320, damping: 30 }}
          className="fixed right-0 top-0 bottom-0 w-[320px] h-screen border-l border-[#2A2A2A] bg-[#0C0C0C] z-30 flex flex-col overflow-y-auto"
        >
      <div className="p-6 pb-2 border-b border-[#2A2A2A]">
        <h2 className="text-[11px] font-medium text-[#CEC5B9] tracking-widest uppercase mb-4">Studio</h2>
...
const WelcomeSection = () => {
  ...
  return (
    ...
    >
      {/* Greeting */}
      <h2 className="text-[42px] font-bold tracking-tight text-[#D4C5A9] mb-1">
        {getGreeting()}
      </h2>
      <p className="text-[14px] text-[#6B6B6B] mb-8">What would you like to do?</p>
...
export default function App() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#0C0C0C] text-[#E5E2E1] font-sans selection:bg-[#D4C5A9] selection:text-[#0C0C0C]">
      ...
      >
        {/* Top App Bar */}
        <header className="sticky top-0 left-0 w-full flex justify-between items-center h-16 px-6 border-b border-[#2A2A2A] bg-[#0C0C0C] z-20">
          <div className="flex items-center gap-4">
            <button
              id="sidebar-toggle"
              onClick={() => setSidebarOpen(o => !o)}
              className="text-[#A0A0A0] hover:text-[#F0F0F0] flex items-center justify-center transition-colors p-1 rounded-md hover:bg-[#1C1C1C]"
              aria-label="Toggle left sidebar"
            >
              <Menu size={18} />
            </button>
            <h1 className="text-[#D4C5A9] font-medium text-[18px] tracking-tight">Notebooks</h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative flex items-center">
              ...
              <input
                className="w-[260px] h-[32px] bg-[#141414] border border-[#2A2A2A] rounded-md pl-8 pr-3 text-[#E5E2E1] placeholder:text-[#6B6B6B] focus:outline-none focus:border-[#A0A0A0] transition-colors text-[13px]"
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto pt-8 px-8 pb-[120px]">
          {/* Welcome Message */}
          <WelcomeSection />
        </div>

        {/* Chat Input Bar */}
        <div className="absolute bottom-4 left-4 right-4 bg-[#1C1C1C] border border-[#2A2A2A] rounded-2xl flex flex-col z-20 h-[56px] justify-center px-4">
          <div className="relative flex items-center w-full">
            <Paperclip className="text-[#6B6B6B] mr-3" size={20} />
            <input
              className="w-full bg-transparent border-none p-0 text-[#E5E2E1] placeholder:text-[#6B6B6B] focus:ring-0 text-[14px] font-normal outline-none"
              placeholder="Ask anything across your notebooks..."
              type="text"
            />
            <button className="w-[32px] h-[32px] bg-[#D4C5A9] rounded-full flex items-center justify-center hover:bg-[#E2D4B9] transition-colors ml-3 flex-shrink-0">
              <ArrowUp className="text-[#0C0C0C]" size={18} />
            </button>
          </div>
        </div>
      </motion.main>
